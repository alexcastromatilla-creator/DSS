...
(async () => {
  ...
  console.log('--- Comprobando el mapa visual (nodos + líneas de conexión + leyenda) ---');
  const mapNodeCount = await page.$$eval('.map-node', els => els.length);
  const territoryCount = await page.$$eval('.territory', els => els.length);
  console.log('Nodos del mapa:', mapNodeCount, '| Territorios listados:', territoryCount);
  if (!mapNodeCount) throw new Error('El mapa SVG no tiene ningún nodo de territorio');
  if (mapNodeCount !== territoryCount) throw new Error('El nº de nodos del mapa no coincide con el nº de territorios listados');
  const lineCount = await page.$$eval('svg line', els => els.length);
  console.log('Líneas de conexión entre territorios:', lineCount);
  if (!lineCount) throw new Error('El mapa no dibuja ninguna conexión entre territorios (no parece un mapa real)');
  const legend = await page.$('.map-legend');
  if (!legend) throw new Error('Falta la leyenda del mapa');

  console.log('--- Tocando un nodo del mapa: debe resaltar su ficha en la lista ---');
  const firstNode = await page.$('.map-node');
  await firstNode.click();
  const flashed = await page.waitForSelector('.territory.flash', { timeout: 1500 }).catch(() => null);
  if (!flashed) throw new Error('Tocar un nodo del mapa no resalta su territorio correspondiente en la lista');
  console.log('OK: tocar un nodo del mapa resalta su ficha en la lista de abajo.');
